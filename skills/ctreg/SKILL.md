...
**한국어 이름과 용어는 지어내지 말고 국내 레지스트리에서 찾아라.** 그곳은 사람·기관·제목·중재를
국문과 영문으로 나란히 싣는다 — 돌아온 영문이 **실제로 등록된 표기**다. 로마자 표기는 사람마다
다르고 다른 곳은 등록된 그 표기로만 걸리므로, 지어낸 철자로 나온 빈 결과는 "그런 연구가 없다"
로 읽힌다. 찾지 못해 추측했다면 **추측했다고 답에 밝혀라.**

## 시나리오별 절차 — 플레이북

자주 오는 물음에는 정해 둔 절차가 있다. `playbooks/` 의 파일 하나가 시나리오 하나다 — 한국어
연구자 이름(`investigator-korean`), 연구자 특징 설명(`investigator-profile`), 순위·비교(`ranking`),
조건으로 찾기(`condition-drug`), 등록번호(`by-id`), 건수 비교(`count-compare`). **물음이 그중 하나에
맞으면 그 파일을 읽고 그 절차를 따르라.** 같은 파일을 공개 웹의 AI 모드도 읽는다 — 어디서 물어도
같은 절차, 같은 한계 표시가 나오게 하려는 것이다.
