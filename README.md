...
**기본이 AI 모드다 — 자연어로 치고 엔터.** "전북대학교병원 김민걸 교수 3상 시험" 을 그대로 치면 모델이
도구와 조건을 고르고(한국어 이름은 `names` 로, 로마자로 옮기지 않는다), **무엇으로 물었는지를
결과 위에 보인다** — 모델이 틀리면 알아챌 수 있게. 이름만 쳐도("김민걸 교수 임상시험") 찾는다 —
CRIS 사본에서 한국어 이름으로 바로 찾고, 거기 **등록된** 영문 표기(오타까지)와 모델이 추측한
표기로 ClinicalTrials.gov 에 각각 묻고, ISRCTN 에는 본문 자유검색으로 물어(이름 축이 없다 — 그래서
연구책임자가 아닐 수도 있다고 밝힌다) 합친다. CTIS 는 이름으로 물을 수 없어 그렇게 표시된다.

**설명을 물으면 답한다.** "김민걸 교수의 임상시험 특징 설명", "주로 수행하는 연구 분야" 처럼
목록이 아니라 답을 원하는 물음이면, 조회 결과를 모델에게 주고 요약을 받아 **결과 위에 "모델의
요약 — 원본은 아래" 로** 보인다. 근거는 조회 결과뿐이다 — 없는 것은 없다고 말하게 했다("한국에서
등수" 는 "이 조회 결과로는 알 수 없다, 전체 연구자 집계가 필요하다"). 근거 레코드는 등록번호로
가리키고, 원본 목록은 그대로 아래 있다. 1~2분 걸릴 수 있다. 어느 표기가 등록된 것이고 어느 것이 추측인지,
표기별로 몇 건인지를 결과 위에 보인다. 모델은 조건 변환만 하고 결과를 요약하거나
해석하지 않는다. AI 모드를 끄면(칩을 누르면, 브라우저가 기억한다) 검색창의 말이 조건으로 바로 간다 — 빠르고 공짜다.
