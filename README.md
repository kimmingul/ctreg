...
**`ctreg results` 는 안 되지만, 결과가 있는지는 알려준다.** CTIS 의 결과는 제출된 요약
**PDF** 다 — `ctreg results` 가 요구하는 구조화된 평가변수 값·이상반응·참가자 흐름·기저
특성이 아니라서 그 커맨드는 exit 3 이다(ISRCTN 과 같은 자리다). 대신 **레코드의
`hasResults` 로 결과 유무를 낸다.** 실측 표본 500건 중 10.8%가 결과를 갖고 있었고, 전부
종료된 시험이었다. 결과가 `Yes`/`No` 로 판별되지 않는 레코드에서는 이 필드가 아예 빠진다 —
**"결과가 없다" 와 "모른다" 를 섞지 않기 위해서다.**
...
이 절도 실측이다. `bun run ctis-field-test` 를 돌리면 `docs/ctis-field-test-<날짜>.md` 로
남는다. **이 레지스트리에서는 그 대조가 특히 중요하다** — 위에 적은 "모르는 키를 조용히
버린다" 가 참이라는 전제 위에 신고가 얹혀 있어서, 그 성질이 바뀌면 이 CLI 는 오류를 내지
않고 **조용히 틀려진다.**
