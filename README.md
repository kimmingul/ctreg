...
Claude Desktop, Cursor 등 **MCP 를 말하는 어떤 호스트**에서든 같은 도구 일곱을 쓸 수 있다.
...
`https://ctreg.trialinsight.ai/` 가 검색 페이지다. 설치 없이 브라우저에서 같은 도구 일곱을 쓴다.
...
**기본이 AI 모드다 — 자연어로 치고 엔터.** AI 모드는 **에이전트**다: Claude Code 에 MCP 를 붙였을 때와
같은 방식으로, 모델이 MCP 와 같은 도구 일곱을 스스로 여러 번 부르고(한 턴에 여럿이면 동시에) 결과를
읽고 다음을 정한 뒤 답한다. 지침도 같다 — 플러그인의 SKILL.md 를 그대로 읽는다(레지스트리 능력 먼저,
경고 읽기, 한국어 이름은 이름 대조부터, 추측했으면 밝히기, 범주별 건수 더하지 않기). 페이지는 도구
호출을 **한 줄씩 실시간으로** 보이고(어느 도구에 무엇을 물어 몇 건이 왔는지), 끝나면 답, 그 아래
도구가 실제로 돌려준 근거 레코드를 둔다. 예: "김민걸 교수의 임상시험 리스트" → 이름 대조 → CRIS 를
한국어로·ctgov 를 표기 둘로 동시에 → 105건, 84초. "당뇨 관련 우수한 연구자 5명" → `ctreg aggregate --by investigator`
(검색어에 걸린 CRIS 시험 전체를 한 축으로 묶어 건수순 — 연구자·의뢰사·실시기관·연도·중재종류·의약품·
질환) 한 번으로 276건 전수 순위 — "우수함은 레지스트리가 판정 못 한다, 등록 건수다" 를 밝힌다.
"어느 의뢰사가 많이 하나", "연도별 추이", "많이 쓰인 약물" 도 같은 도구다. ctgov 로도 되지만 집계 API 가
...
`ctreg-mcp-http` 가 같은 도구 일곱을 **Streamable HTTP** 로 낸다. 클라우드에 띄우면 사용자는
설치 없이 URL 하나로 붙는다:
