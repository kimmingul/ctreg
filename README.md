...
로마자 표기 후보를 여럿 만들어 ClinicalTrials.gov 에 각각 묻고 합친 뒤, 그 시험들의 장소에서
소속 기관을 읽어 CRIS 에서 그 기관의 시험 연구책임자를 대조한다. 다만 **추측한 표기·소속이라고
밝히고** 어느 표기가 몇 건이었는지 보인다. 소속 기관이나 연구 주제를 함께 적으면 추측 없이
CRIS 에서 실제 등록된 표기를 읽어 대조한다. 모델은 조건 변환만 하고 결과를 요약하거나
