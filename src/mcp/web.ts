 ...
 * 새로 쓰지 않는다.
 *
 * **모델이 끼지 않는다.** 검색창에 친 말이 조건으로 바로 간다. 그래서 빠르고 공짜이고,
 * "조용히 틀린 답" 이 끼어들 자리가 없다. 자연어를 조건으로 바꾸는 일은 MCP 를 쓰는 모델의
 * 몫이지 이 페이지의 몫이 아니다 — 설계 스펙 §1.2 의 비목표(합성 워크플로)와 같은 이유다.
 *
 * **고급 검색의 칸은 손으로 적지 않는다.** `/api/schema` 가 `OPTION_HELP` 를 그대로 내고
 * 페이지가 그것으로 칸을 그린다 — MCP 스키마와 `--help` 가 읽는 그 표다.
 */
