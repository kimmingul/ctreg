  ...
  if (q.phase?.length) {
    const values = q.phase.map((p) => {
      ...
      if (!out) {
        ...
        // 물어볼 수 없다" 이므로 거절한다.
        //
        // **여기까지 오지 않는다(2026-08-27 실측).** 가드가 요청 값을 그 레지스트리가
        // 신고한 `values` 에 대조해 먼저 막고, 사용자가 보는 것은 **exit 3** 이다.
        // 이 줄이 한때 "exit 2 다" 라고 적었던 것은 두 번 틀렸다: 가드가 먼저 서고,
        // 그 전에도 이 오류의 `exit` 필드는 레지스트리별 봉투로 접혀 프로세스 코드가
        // 되지 않았다(`exitFor` 가 4로 접는다). 방어선으로 남긴다 — 어댑터를 직접
        // 부르는 자리(필드테스트·스크립트)에는 가드가 없다.
        throw usageError(
