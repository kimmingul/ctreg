...
export const CRIS_CAPABILITY: Capability = {
  ...
  search: {
     ...
     */
    /**
     * **완전성을 약속하지 못한다** — 후보가 `--term` 이 닿는 범위에 갇히기 때문이다.
     * 실측 2026-08-28: 검색어 하나(`전북대학교병원`, 후보 177)로는 40건 중 12건만 나왔고,
     * 검색어를 여덟 개로 늘려 후보를 604건으로 넓히니 40건이 나왔다. 그의 시험 대부분은
     * 연구책임기관이 제약사라 기관명 하나로는 모이지 않는다.
     *
     * `srchWord` 가 정확히 무엇을 색인하는지는 **모른다.** 연구비지원기관·연구책임기관·제목에
     * 닿는 것은 봤지만 규칙이 단순하지 않고, 표본 하나로 단정했다가 한 번 틀렸다.
     * 그래서 이 축은 "찾은 것" 만 말하고 "전부" 를 말하지 않는다.
     */
    investigator: free(
      '목록으로는 못 거른다 — --term 으로 좁힌 후보를 상세 조회로 하나씩 열어 연구책임자 이름을 대조한다. ' +
        '후보는 검색어가 닿는 범위(연구책임기관·제목)에 갇히므로 전수가 아니다',
    ),
    geo: off('좌표 검색이 없다'),
    /**
   ...
   * 의뢰기관·제목·연도뿐이다. 국내 집계는 KCTIS 의 MCP 가 한다(ctreg 밖). 신고를 두는 이유: 신고가 없으면 커맨드가 검색을
   * 걸어 세려 들고, 그 결과는 축의 절반이 빈 채 "0건" 으로 보인다.
   */
  aggregate: { supported: false, scope: '공식 API 목록에는 연구책임자·실시기관·질환 항목이 없다. 국내 집계는 KCTIS 의 MCP(읽기 전용 SQL)가 한다 — ctreg 밖이다', axes: [] },
