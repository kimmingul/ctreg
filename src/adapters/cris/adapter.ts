...
export const CRIS_CAPABILITY: Capability = {
  ...
  search: {
     ...
     */
    /**
     * 실측 2026-08-28: `srchWord` 는 연구책임기관과 제목에 닿고 **참여기관에는 닿지 않는다.**
     * `KCT0000145`(연구책임자 기관이 전북대학교병원)가 `전북대학교병원` 177건에도
     * `전북대학교` 237건에도 없다 — 그 시험의 연구책임기관이 동화약품이기 때문이다.
     * 그래서 이 축은 **완전성을 약속하지 못한다.** 경고가 그 사실을 함께 낸다.
     */
    investigator: free(
      ...
      if (q.investigator !== undefined) {
        ...
        warnings.push({
          ...
          message:
            ...
            '**걸린 수는 이 연구자의 전부가 아닙니다**: 후보는 --term 이 닿는 범위에 갇히는데, ' +
            '그 검색어는 연구책임기관과 제목에는 닿지만 참여기관에는 닿지 않습니다. ' +
            '기관명으로 좁히면 그 기관이 연구책임기관인 시험만 후보가 되므로, ' +
            '같은 연구자가 다른 기관 의뢰로 수행한 시험은 빠집니다. 이 쪽의 후보만 본 것이기도 합니다.',
          registry: 'cris',
        });
        data = kept;
      }
