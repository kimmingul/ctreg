...
## 정정 — "어느 값에도 안 걸리는 수" 는 phase 에 대해 틀린 이름이다 (2026-08-23, 최종 리뷰 H3)

위 표의 마지막 열은 `전체 총계 − 값별 합` 을 그대로 찍은 것인데, 헤더가 그것을 **"어느
값에도 안 걸리는 수"** 라고 불렀다. 겹치지 않는 축(status·studyType)에서는 두 수가 같지만
**phase 에서는 같지 않다.** 어댑터의 scope 가 적어 둔 대로 "여러 단계를 신고한 시험은 그
전부에 걸리"므로, 값별 합에는 중복이 들어 있다. 중복이 있으면 실제로 덮인 시험 수는 합보다
작고, 따라서 **어디에도 안 걸리는 진짜 수는 117,554 보다 크다.** 위 표의 117,554 는 뺄셈의
결과일 뿐 실측된 미포함 시험 수가 아니다.

같은 이유로 겹치는 축에서는 `합 >= 총계` 도 덮개를 증명하지 못한다. 스크립트는 이제 그
열을 `전체 − 값별 합` 으로 부르고, 겹치는 축은 `true` 로 판정하지 않으며, 판정을 어댑터의
`exhaustive` 선언과 대조해 어긋나면 실패로 집계한다(이 실행에는 그 대조가 없었다 — 위
`집계` 줄의 13/0/1 은 덮개 표를 세지 않은 수다).

숫자는 실행 당시 측정된 것이므로 고치지 않는다.
