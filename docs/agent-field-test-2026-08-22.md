...
4. **F2·F5·F8** — 같은 뿌리. 선언이 존재만 말하고 내용을 말하지 않는다. F8 은 한 걸음 더 나아가 어휘 자체가 데이터를 덮지 못한다 — 열거할 수 없는 것이 아니라 **표현할 수 없는 값이 있다.**
5. **F6·F3** — 마찰이지 오답이 아니다.

**F1·F2·F4·F5·F8 은 하나의 패턴이다** — 선언이 불완전하면 에이전트는 한도를 부딪혀서만
