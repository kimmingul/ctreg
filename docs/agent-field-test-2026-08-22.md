...
| F2 | 능력 선언이 존재만 말하고 적용 범위를 말하지 않는다 | **CLI-FIX** | `term: true` 는 축의 존재만 선언한다. 0건이 "없음"인지 "안 봄"인지 판별 근거가 도구 안에 없다. **해소됨 — 아래 「CLI-FIX 우선순위」 5번의 각주에 커밋이 있다.** |
| F3 | 서브커맨드별 `--help` 가 없다 | **CLI-FIX** | 최상위와 바이트 단위로 동일. 세 시나리오가 부딪혔다. **해소됨 — 커밋 `a1473e3`.** |
| F4 | 경고가 존재하지 않는 복구 경로를 암시한다 | **CLI-FIX** | "잘렸다"만 말하고 "되찾을 수 없다"를 안 말해 에이전트가 없는 해결책을 권했다. 실측: `get` 은 `--page-size`·`--page-token`·`--sort` 를 받아들이고 **exit 0 으로 조용히 무시**한다(위치는 10개 그대로, 경고 없음). **해소됨 — 커밋 `ceba94c`(경고가 복구 경로를 말한다) + `5b15ec1`(안 쓰는 플래그는 exit 2).** |
| F5 | 어휘를 열거할 방법이 없다 | **CLI-FIX** | 에이전트가 틀린 값을 일부러 넣어 힌트를 캤고, 나머지는 추측했다. **해소됨 — 아래 「CLI-FIX 우선순위」 5번의 각주에 커밋이 있다.** |
| F6 | `results_summarized` 가 부분 전개를 구분하지 않는다 | **CLI-FIX** | 전개 전과 부분 전개가 같은 코드. **해소됨 — 커밋 `fa19c1e`.** |
| F8 | 어휘가 데이터를 덮지 못한다 | **CLI-FIX** | `na` 와 "phase 필드 자체가 없음"은 다른 것인데 후자를 가리킬 값이 없다. 업스트림 표본 1000건 중 필드 부재 52건. 범주별 건수가 항상 총계에 못 미치고 모자란 부분에 이름을 붙일 수 없다. **해소됨 — 아래 「CLI-FIX 우선순위」 5번의 각주에 커밋이 있다.** |
| F7 | 파이프가 일찍 닫히면 크래시한다 | **CLI-FIX** | exit 1(계약에 없는 코드) + 원시 스택트레이스. **해소됨 — 커밋 `a3cce54`.** |
| F9 | 대문자 값 거부 → 힌트 → 자력 복구 | **CLI-FIX** (정정) | 처음엔 AGENT(설계 성공)로 적었으나 **잘못이다.** S2 가 받은 것과 **문자 그대로 같은 힌트**다. 같은 도구 동작이 시나리오에 따라 다른 판정을 받을 수 없다. `--help` 는 세 경우 모두 대소문자 규칙도 값 어휘도 말하지 않고 **틀린 값을 넣었을 때만** 말한다 — F8 이 지적한 그 패턴이다. **해소됨 — 아래 「CLI-FIX 우선순위」 5번의 각주에 커밋이 있다.** |
...
| F12 | `--radius` 기본값이 선언되지 않는다 | **CLI-FIX** | S1: *"50km 는 임의로 고른 값 — 도구에 기본값이 없어 이 선택이 답의 경계를 좌우한다."* 답의 범위를 정하는 값이 문서화되지 않았다. **해소됨 — 아래 「CLI-FIX 우선순위」 7번의 각주 참조(커밋 해시는 기록되지 않았다).** |
| F13 | `--full` 이 organ 필터를 덮어쓴다 | **CLI-FIX** | S3 이 `--full` 로 arm 별 분리를 찾다가 organ 필터가 무시되는 것을 발견했다. 경고 없이 무시된다. **해소됨 — 아래 「CLI-FIX 우선순위」 6번의 각주에 커밋이 있다.** |
| F14 | `registries` 의 `results: true` 가 오독을 부른다 | **CLI-FIX** | S6 이 이것을 "결과 게시 여부로 검색 가능"으로 읽었다. 실제로는 `results` 서브커맨드 지원을 뜻한다. F2 와 같은 뿌리. **해소됨 — 아래 「CLI-FIX 우선순위」 5번의 각주에 커밋이 있다.** |
...
| O2 | 재현되지 않는 `data: null` | **부류 닫힘** | 두 번 관찰, 세 가설 기각, 재현 실패. **재현 대신 `data: null` 을 내는 다섯 자리를 빠짐없이 세어 전부 이유를 함께 싣는다는 것을 테스트로 못 박았다**(커밋 `e3e2ecf`) — 조용한 null 은 이제 구조적으로 나올 수 없다. 근거는 `docs/slice-2-prerequisites.md` 의 O2 절. |
...
5. **F2·F5·F8·F9·F14** — 같은 뿌리. 선언이 존재만 말하고 내용을 말하지 않는다.
   ...
   >
   > **F2 도 해소 (2026-08-25) — 커밋 `920fb7f`.** 예고한 "별도 변경" 이 그것이다.
   > 결과가 0건일 때만 그 질의가 쓴 축의 `scope` 가 `zero_results_scope` 경고로 붙는다.
   > 빌드한 CLI 로 재확인: `search --registry ctgov --term "2015-000397-19"` 는 이제
   > 0건 · **경고 1건** · exit 0 이고, 걸리는 질의(`--term pembrolizumab`)에서는 침묵한다.
   > 종료 코드는 그대로다 — 0건은 여전히 오류가 아니고, 달라진 것은 그 0 이 스스로를
   > 설명한다는 것뿐이다.
   >
   > 아래는 당시의 진단으로 남긴다. **F2 만 다르다.** `scope` 는 선언에 근거를 실었지만
   > **응답에는 싣지 않았다.** 빌드한 CLI 로 재확인: `search --registry ctgov --term
   > "2015-000397-19"` 는 결과 0건 · 경고 없음 · exit 0 — 브랜치 이전과 같다. 다섯 중
   > 실패 경로가 아직도 **exit 0 인 0건** 으로 끝나는 것은 F2 뿐이다. 자유 텍스트 축의
   > 0건에 붙는 경고는 별도 변경이다(F8 이 받은 사용 시점 경고와 같은 자리·같은 모양이
   > 될 것이다).
6. **F6·F11·F13** — 경고와 플래그의 의미가 불명확하다.

   > **F13·F11 해소 (2026-08-24).** `--full` 은 **요약 여부**, 필터는 **선택 여부** 로
   > 의미를 갈랐다 — 둘을 같이 주면 이제 필터가 이긴다(전에는 `--full` 이 필터를 삼켜
   > 좁혀 달라는 요청에 넓은 답이 경고 없이 돌아갔다). `locations_truncated` 는 무엇이
   > 잘렸는지에 더해 **되찾는 법**(`--include locations`)까지 말한다.
   > **F6 도 해소 (2026-08-25) — 커밋 `fa19c1e`.** 불리언 하나를 두 수(펼친 수·펼칠 수
   > 있었던 수)로 바꿔 두 상황을 갈랐다. 일부만 펼친 경우는 새 코드
   > `results_partially_expanded` 가 몇 개 중 몇 개가 남았는지를 말한다 — 옛 문구는 이
   > 경우 **거짓** 이었다(이미 필터를 쓴 호출자에게 필터를 쓰라고 말했다).

7. **F3·F12** — 마찰이지 오답이 아니다.

   > **F3 해소 (2026-08-25) — 커밋 `a1473e3`.** `--help` 가 커맨드 단어를 버리고
   > `registries` 로 바꿔치기하던 것이 원인이었다. 그 단어를 살리고, F4 가 만든
   > `COMMAND_OPTIONS` 표를 그대로 읽어 커맨드별 표면을 낸다 — 거절하는 쪽과 안내하는
   > 쪽이 한 표를 본다.

   > **F12 해소 (2026-08-24).** 기본 반경 50km 를 `geo.scope` 에 실었다 — `registries`
   > 가 묻기 전에 말한다. 적용된 뒤에 알려주는 `geo_radius_defaulted` 경고는 그대로다.
   > **F3 은 남아 있다** — 서브커맨드별 `--help` 가 아직 없다.
