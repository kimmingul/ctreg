...
| **EU CTIS** | ✅ **붙였다(2026-08-30, 어댑터 #5 `ctis`).** 접근 조건까지 확인했다: `euclinicaltrials.eu` 에 `robots.txt` 가 없고, EMA 법적 고지가 재생산·배포를 상업·비상업 모두 허용하며 조건은 출처 표시 하나다(*"included in each copy"* — 그래서 레코드마다 `attribution` 을 싣는다). 실제로 거르는 검색 키는 넷뿐이고 나머지는 **조용히 무시된다**(있지도 않은 키를 보내도 전체가 온다) — 자세한 것은 `slice-2-prerequisites.md` 의 「어댑터 #5」 절. 아래는 당시의 조사로 남긴다. **정정(2026-08-27): 인증 없는 공개 REST API 가 있다.** 원래 판단은 "화면만 확인함" 이었는데, 공개 포털이 쓰는 API 를 직접 쳐서 확인했다 — `POST https://euclinicaltrials.eu/ctis-public-api/search` 에 `{"pagination":{...},"searchCriteria":{}}` 를 보내면 **12,308건**이 JSON 으로 오고, `ctis-public-api/retrieve/{id}` 도 있다(엔드포인트는 포털 번들 `main-*.js` 의 인터셉터 정규식에서 찾았다). EMA 가 **문서로 공개한** REST API 를 못 찾았다는 원 판단 자체는 여전히 맞다 — 문서화되지 않은 포털 내부 API 라 계약이 없다. 아래는 당시의 진단으로 남긴다. **화면(공개 포털)만 확인함.** EMA가 공식 REST API를 문서로 공개한 것을 찾지 못했다 — 서드파티 스크레이퍼(Apify 등)만 존재. | 포털 열람 자체는 불요(회원가입 없이 조회 가능) | [CTIS public portal: Full trial information (EMA/441147/2024, 2025-01-27)](https://www.ema.europa.eu/en/documents/other/clinical-trial-information-system-ctis-public-portal-full-trial-information_en.pdf) · [CTIS public portal: summary (EMA/441149/2024, 2024-09-20)](https://www.ema.europa.eu/en/documents/other/clinical-trial-information-system-ctis-public-portal-summary_en.pdf) · 포털: `https://euclinicaltrials.eu` |
...
| dates.lastUpdated | CT.gov·ISRCTN·CRIS·jRCT·**CTIS** 확정 O(CTIS 는 2026-08-27 실측으로 레코드별 확정), ICTRP `~`(필드는 있으나 **수확일** 이라 뜻이 다르다) | **core 유지** (확정 4곳으로 규칙 충족) |
...
| ~~contacts(detail)~~ | CRIS | **확인됨(2026-08-29) — 있다, 그리고 두 역할로 나뉜다.** 상세 조회(`/detail`, 파라미터는 `crisNumber`)가 연구책임자(`scientific_name_kr`/`_en`, `affiliation_kr`/`_en`)와 연구실무담당자(`public_name_kr`/`_en`, `public_affiliation_kr`/`_en`, `telephone`)를 따로 낸다. 실측 KCT0000145: `김민걸` / `Min gul Kim` / 전북대학교병원, 실무담당자 `한수미`. **두 역할을 섞으면 안 된다** — 섞었다가 남의 연구가 목록에 들어간 적이 있다(ctreg `197d288`). | (해소됨) |
