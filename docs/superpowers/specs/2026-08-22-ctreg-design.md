...
**조용한 절단은 금지한다.** 잘라낸 모든 지점은 `warnings[]` 항목 하나와, 레코드 안의 `*Truncated` 플래그나 `*Total` 개수 중 하나를 남긴다(예: `eligibility.criteriaTruncated`, `locationsTotal`, `outcomesTotal`). 조용한 절단은 에이전트를 가장 확실하게 속이는 실패 모드다.
...
3. **`filter.ids` 의 배치 상한 및 URL 길이 한계** — 초판은 합성 NCT ID(`NCT0428xxxx`, 대부분 미존재)로 시험해 "긴 URL이 받아들여진다"만 증명하고 배치 자체는 시험하지 못했다. 재검토에서 실제 검색 결과에서 모은 진짜 NCT ID로 50/100/200/300/500개를 늘려가며 `countTotal` 이 요청 개수와 정확히 일치하는지 확인했고, **500개까지 전부 정확히 일치**했다(잘림 없음, 오류 없음). 즉 CT.gov 쪽 실측 상한은 최소 500 이상이며 잠정값 50 보다 한참 위다. 500 을 넘는 지점은 시도하지 않았다 — "상한이 없다"는 뜻이 아니라 "적어도 500까지는 없다"는 뜻이다.
   **그래도 `CTGOV_CAPABILITY.limits.maxBatchIds: 50` 은 그대로 둔다 — CT.gov 의 상한이 아니라 우리 쪽 `get()` 구현의 상한 때문이다.** `get()` 은 배치 하나당 요청을 한 번만 보내고 응답을 페이지네이션하지 않는데, `buildIdsParams` 가 `pageSize` 를 `CAPS.pageSize.max`(200)로 캡핑한다. 그래서 `maxBatchIds` 를 200 넘게 올리면 그 초과분은 요청은 되지만 응답에는 실리지 않고 경고 없이 조용히 사라진다 — 호출자는 500개를 물어보고 200개를 전체 결과로 오인하게 된다. 이 불변식은 이제 계약 스위트(`tests/contract/adapter-contract.ts`, "maxBatchIds 는 한 페이지로 전부 읽을 수 있는 범위를 넘지 않는다")가 `maxBatchIds ≤ CAPS.pageSize.max` 로 강제한다. 50 을 올리려면(최대 slice 2 에서 200 까지) 먼저 `get()` 에 배치 내부 페이지네이션을 넣거나 `CAPS.pageSize.max` 에서 멈춰야 한다 — 응답 크기 트레이드오프가 있는 별도 결정이라 이번 슬라이스에서는 하지 않는다.
