...
- **일자:** 2026-08-26
- **상태:** 구현됨 — 커밋 `9c004d5..26dcfc8`. **첫 판 뒤에 두 가지가 더 열렸다:**
  `location`(§2.3)과 `get`(§3.3). 둘 다 "먼저 재고 켠다" 는 조건을 실측으로 충족한 뒤였다.
...
### 3.3 `get` 은 열렸고 `results` 는 아직이다

> **`get` 은 열렸다(2026-08-27).** 아래가 "막혀서가 아니라 파싱 면적을 줄이려고 미룬다" 고
> 적은 그대로, 면적을 재고 나서 열었다. 레코드 화면은 WHO TRDS 24항목을 싣고 레지스트리
> 다섯 곳(NCT·ISRCTN·ChiCTR·ACTRN·JPRN)에서 마크업이 같다. 그래서 `get` 으로 온 레코드는
> 검색 결과 행보다 훨씬 충실하고, **특히 상태가 이진이 아니다** — 검색 경로에서 `other` 였을
> 시험이 여기서는 `completed` 다. 같은 시험이 경로에 따라 다른 충실도로 오는 것은 사실이고
> README 가 그것을 말한다.
>
> 이것으로 **약관의 처리일 표시 의무가 닫혔다** — `sourceRefreshedAt`(§7)이 드디어 채워진다.
> 검색 화면에는 그 값이 없어 `search` 경로에서는 여전히 비어 있다.
>
> **빈 껍데기 페이지가 실재한다.** 내용이 없는 시험에도 200 과 함께 섹션 제목만 있는 페이지가
> 온다(23,501바이트 고정, 표본 11건 중 2건). 레코드를 지어내면 스키마도 어기고 "그 시험은
> 이렇다" 는 거짓이 되므로 `not_found` 로 신고한다.
>
> **대가:** 배치 창구가 없어 ID 하나당 요청 하나이고 `ratePerSec: 1` 이라 10개면 10초쯤이다.
> `detail`(적격기준 전문·평가변수·연락처)은 여전히 `false` 다 — 섹션 파싱 규칙이 따로 필요하고,
> 지금 방식(라벨 다음 줄이 값)은 여러 줄 값을 첫 줄만 잡는다.
>
> 아래는 첫 판 당시의 판단으로 남긴다.

`capability.get`·`capability.results` 를 `supported: false` 로 신고한다.
`Trial2.aspx?TrialID=<id>` 가 안정적인 GET 이라 `get` 은 다음 판에 어렵지 않게 열 수 있다 —
**막혀서가 아니라 파싱 면적을 줄이려고 미룬다.**

> **UA 게이트는 없다(실측 2026-08-26).** 한때 맨몸 fetch 가 200 "Error Page" 를 받고 브라우저
> UA 로는 레코드가 와서 UA 게이트를 의심했는데, ID 셋 × UA 둘 = 6회를 재시도와 함께 다시 재니
> **여섯 번 다 레코드가 왔다**(plain 75,772/59,342/87,172바이트, browser 77,652/62,524/89,598).
> 앞서 본 실패는 그 시간대의 사이트 불안정이었다 — 같은 시각 다른 프로브가 HTTP 500 을 받았다.
> 즉 `get` 을 열 때 특별한 헤더는 필요 없다.
>
> 이 관찰은 **단발 실패를 게이트의 증거로 삼지 않는다** 는 규율이 실제로 값을 한 사례다.
> 처음 결론대로였다면 있지도 않은 UA 게이트를 우회하는 코드를 넣을 뻔했다. `results` 는 ICTRP 가 구조화된 결과 데이터를
...
- ~~`get`~~ — **열렸다(§3.3).** `results` 는 여전히 `supported: false` 다
