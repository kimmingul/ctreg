...
## 정정 — 28592 는 레지스트리 전체가 아니라 하한이다 (2026-08-23, 최종 리뷰 H2·H3)

위 "전체 총계" 열의 28592 는 `lastEdited GE 1900-01-01T00:00:00` 으로 잰 수다. 날짜 필터는
그 날짜를 기재한 레코드만 매칭하므로 — 이 저장소가 `date_filter_excludes_missing` 으로 계속
경고하는 바로 그 규칙 — `lastEdited` 가 없는 레코드는 이 수에서 빠진다. 즉 **정확한 총계가
아니라 하한이다.**

방향이 문제다. 낮춰 잡힌 총계는 `값별 합 >= 총계` 를 **더 쉽게** 만들어 덮개를 부풀린다.
studyType 의 여유는 59건(28533 vs 28592)뿐이라, 하한이 그만큼만 낮춰 잡혀도 부등호가
뒤집혀 `exhaustive: true` 가 된다. 그래서 스크립트는 이제 이 수로 `true` 를 결론짓지 않는다 —
`합 < 하한` 일 때만 확정 `false` 이고, 나머지는 판정 불가다. 아울러 마지막 열은 "어느 값에도
안 걸리는 수" 가 아니라 `전체 − 값별 합` 이다(총계가 하한이고 phase 는 결합 값을 쓰므로
양쪽에서 틀어진다).

이 실행에는 선언과의 대조가 아예 없었다 — 표를 찍고 멈췄다. 지금은 어긋나면 실패로
집계되고 종료 코드가 바뀐다. 숫자는 실행 당시 측정된 것이므로 고치지 않는다.
