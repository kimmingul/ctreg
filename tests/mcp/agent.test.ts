...
describe('플레이북', () => {
  it('파일마다 이름·언제·절차가 있고, 목록이 그것에서 나온다', () => {
    ...
    expect(list.map((p) => p.name).sort()).toEqual(['by-axis-analysis', 'by-id', 'condition-drug', 'count-compare', 'export-list', 'investigator-korean', 'investigator-profile', 'ranking']);
